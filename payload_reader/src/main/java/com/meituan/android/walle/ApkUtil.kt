...
object ApkUtil {
    ...
    @Throws(IOException::class)
    fun findCentralDirStartOffset(fileChannel: FileChannel): Long {
        return findCentralDirStartOffset(fileChannel, getCommentLength(fileChannel))
    }

    @Throws(IOException::class)
    fun findCentralDirStartOffset(fileChannel: FileChannel, commentLength: Long): Long {
        // End of central directory record (EOCD)
        // Offset    Bytes     Description[23]
        // 0           4       End of central directory signature = 0x06054b50
        // 4           2       Number of this disk
    ...
    @Throws(IOException::class, SignatureNotFoundException::class)
    fun findApkSigningBlock(fileChannel: FileChannel, centralDirOffset: Long): Pair<ByteBuffer, Long> {
        ...
        if (apkSigBlockSizeInFooter < footer.capacity() || apkSigBlockSizeInFooter > Int.MAX_VALUE - 8) {
            throw SignatureNotFoundException("APK Signing Block size out of range: $apkSigBlockSizeInFooter")
        }
        val totalSize = (apkSigBlockSizeInFooter + 8).toInt()
        val apkSigBlockOffset = centralDirOffset - totalSize
        if (apkSigBlockOffset < 0) {
            throw SignatureNotFoundException("APK Signing Block offset out of range: $apkSigBlockOffset")
        }
    ...
    fun findIdValues(apkSigningBlock: ByteBuffer): Map<Int, ByteBuffer> {
        ...
        while (pairs.hasRemaining()) {
            ...
            if (lenLong < 4 || lenLong > Int.MAX_VALUE.toLong()) {
                throw SignatureNotFoundException(
